...
void openTheDoor(unsigned long cardID){
  for(int i=0;i<sizeof(ids)/4;i++){
    if(cardID==ids[i]){
      Serial.println("Check OK, open the door");
...
void loop()
{
    ...
    if (status == MI_OK)
    {
        ...
        memcpy(serNum, str, 5);
        unsigned long cardID = ShowCardID(serNum);
        openTheDoor(cardID);
    }
 ...
 * Return：Null
 */
unsigned long ShowCardID(uchar *id)
{
    int IDlen=4;
    unsigned long cardID = 0x00000000;
